...
public class BasCase4 {
    public void method () throws IOException {
        ...
        System.out.println("Score time 4");
        String s1t4 = br.readLine();

        //Score correctness check

        if (s1t1.length()>=3 & s1t1.length()<6 & s1t2.length()>=3 & s1t2.length()<6 & s1t3.length()>=3 & s1t3.length()<6 & s1t4.length()>=3 & s1t4.length()<6) {
            ...
            //Checkforspace for the fourth time

            int checkforspacet4 = 0;
            for (int i=0; i<s1t4.length();i++) {
                if (Character.isDigit(s1t4.charAt(i))) {
                } else checkforspacet4 = i;
            }
            ...
            int [] i1t4 = new int [checkforspacet4];
            for (int i=0; i<checkforspacet4;i++) {
                i1t4[i] = Integer.parseInt(String.valueOf(s1t4.charAt(i)));
            }
            int sc14 = 0;
            for (int i = 0; i < checkforspacet4; i++)
                sc14 = sc14 * base + i1t4[i];
            ...
            int [] i2t4 = new int [s1t4.length()-checkforspacet4-1];
            for (int i=0; i<s1t4.length()-checkforspacet4-1;i++) {
                i2t4[i] = Integer.parseInt(String.valueOf(s1t4.charAt(i+checkforspacet4+1)));
            }
            int sc24 = 0;
            for (int i = 0; i < s1t4.length()-checkforspacet4-1; i++)
                sc24 = sc24 * base + i2t4[i];

            //Processed score outputs

            System.out.println("Final Score is "+(sc11+sc12+sc13+sc14)+":"+(sc21+sc22+sc23+sc24));
            if ((sc11+sc12+sc13+sc14)>(sc21+sc22+sc23+sc24)){
            ...
            System.out.println("Motherfucker!!! That's my last warning, bitch!");
        }
    }
}